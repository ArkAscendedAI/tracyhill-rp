...
function codexArgs({ prompt, sessionId, cwd, imagePaths }) {
  ...
  if (sessionId) return ["exec", "-s", "danger-full-access", "resume", "--json", "--skip-git-repo-check", ...images, sessionId, prompt];
  return ["exec", "--json", "--skip-git-repo-check", "-s", "danger-full-access", "--cd", cwd, ...images, prompt];
}
